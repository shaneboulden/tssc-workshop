...
```bash
podman login -u ${USER} -p $(oc whoami -t) registry.blueradish.net
```
Pull down the test app, and re-tag it using your Red Hat Quay handle
```bash
...
```bash
cosign sign registry.blueradish.net/$(oc project -q)/demo-app@<digest for your image>
```
