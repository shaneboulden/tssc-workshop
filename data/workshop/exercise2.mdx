...
Inside the terminal session login to the registry:
```
podman login --authfile=auth.json registry.blueradish.net
```
...
* `image-name`: Update this to reflect the internal registry and your current namespace. e.g. `registry.blueradish.net/user1/chat-client:latest`
...
Login to the local registry and 

Navigate back to the terminal window and check that you can verify the container image with `cosign`:
```
cosign verify --key k8s://openshift-pipelines/signing-secrets quay.blueradish.net/$(oc project -q)/chat-client:latest
```
