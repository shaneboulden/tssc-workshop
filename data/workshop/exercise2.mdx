...
In this lab we'll sign a container image with Tekton Chains, push this to a container registry, and then validate the attestations created about the container image and verify the container signatures.
...
```
oc create secret generic regcred --from-file=.dockerconfigjson=~/.config/containers/auth.json --type=kubernetes.io/dockerconfigjson
```
...
```
oc adm policy add-role-to-user isview -z pipeline-svc --role-namespace=$(oc project -q) -n $(oc project -q)
