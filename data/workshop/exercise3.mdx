...
In both these exercises we manually verified container signatures using the `cosign` and `rekor-cli` binaries. This is effective, but security teams need an automated way of verifying signatures. Every time a user tries to deploy a container image, we should check that it is signed by an appropriate key, and if not, block the image from the platform.
...
Name the new policy `Cosign verification - <your username>`. Give it a severity of `Medium`, and select `Kubernetes` from the Categories. You don't need to add anything for the description, rationale or guidance, but you can complete these sections if you'd like.
